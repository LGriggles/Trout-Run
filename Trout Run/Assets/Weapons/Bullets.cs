...
public class Bullets : MonoBehaviour
{
    ...
    {
        bool hitSummat = false;
        int numParticles = _bullets.GetParticles(_particles);
        if (numParticles == 0) return; // no bullets here!

        // For each particle (bullet), check if it collides with something
        ...
        if (hitSummat)
            _bullets.SetParticles(_particles, numParticles);
    }
}
