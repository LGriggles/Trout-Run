...
public class LevelEditor : Editor
{
	...
	[MenuItem("Level Editor/New Segment")]
	static public void CreateSegment()
	{
		// Create new Level
		GameObject newLevel = new GameObject();
		newLevel.name = "Level Segment";
	...
	public override void OnInspectorGUI()
	{
		...
		_level.currentMode = EditorGUILayout.Popup(_level.currentMode, LE_Mode.levelLabels);
		...
		if(_faceSelected)
		{
			...
			GUILayout.Label (LE_Mode.levelLabels[_level.currentMode] + " Tools", EditorStyles.boldLabel);
			...
			case LE_Mode.FACE:
				...
				_level.faceExtrude = EditorGUILayout.Toggle ("Extrude", _level.faceExtrude);
			...
			case LE_Mode.EDGE:
				...
				_level.edgeLoopMove = EditorGUILayout.Toggle ("Move Loop", _level.edgeLoopMove);
		...
		{
			GUILayout.Label ("Ctrl Click to select a " + LE_Mode.levelLabels[_level.currentMode].ToLower());
		}
	...
	void OnSceneGUI()
	{
		...
		if(_faceSelected)
		{
			...
			if(_level.faceMove || _level.edgeMove || _level.edgeLoopMove)
			{
			...
			// EXTRUDING!
			if(_level.faceExtrude)
			{
				// Sliders, that's how we roll here at Wonga(tm)
				Handles.color = new Color(1, 0.6f, 0.05f);
				vertOffset = Handles.Slider(globalCentre, _level.transform.TransformDirection(_currentFace.normal), HandleUtility.GetHandleSize(globalCentre) * 0.8f, Handles.ArrowCap, 1);
				offsetVector = Vector3.Scale(_level.transform.InverseTransformDirection(vertOffset - globalCentre), _normalMask); // scale by normal ensures only correct direction is modified
			...
			if(Event.current.type == EventType.MouseDown)
			{
				...
				case 0:
					...
					if (Physics.Raycast(mouseRay.origin, mouseRay.direction, out hit, Mathf.Infinity))
					{
						...
						_currentEdgeIndex = _currentFace.NearestEdge(_level.transform.InverseTransformPoint(poi)); // nearest edge to click

						// Set split edge if in split mode
						if(_level.faceSplit) CalcSplitEdge(poi);
						...
						*/
						...
						//_currentFace.PrintInfo();
						/*
						Vector3[] verts = _currentFace.verts;
						...
						*/
						//Debug.Log (triIndex);
						//Debug.Log (offsetToTile(_currentFace.tileOffset));
	...
	private Mesh GenerateMesh()
	{
		...
		AssetDatabase.CreateAsset(CreateCube(), fileName);
	...
	private Mesh CreateCube() 
	{
		...
		/* HERE COMES THE CUBEY BIT! */
		Vector3[] verts = new Vector3[24];
		Vector2[] uvs = new Vector2[24];
		Vector2[] offsetUvs = new Vector2[24];
		int[] tris = new int[36];
		...
		verts[0] = new Vector3(-0.5f, 0.5f, -0.5f); // Front Top Left
		verts[1] = new Vector3(0.5f, 0.5f, -0.5f); // Front Top Right
		verts[2] = new Vector3(-0.5f, -0.5f, -0.5f); // Front Bottom Left
		verts[3] = new Vector3(0.5f, -0.5f, -0.5f); // Front Bottom Right

		verts[9] = new Vector3(-0.5f, 0.5f, 0.5f); // Back Top Left
		verts[8] = new Vector3(0.5f, 0.5f, 0.5f); // Back Top Right
		verts[11] = new Vector3(-0.5f, -0.5f, 0.5f); // Back Bottom Left
		verts[10] = new Vector3(0.5f, -0.5f, 0.5f); // Back Bottom Right
		...
		cubeMesh.uv = uvs;
		cubeMesh.uv2 = offsetUvs;

		cubeMesh.RecalculateNormals(); // might do this by hand later.. (please don't...)
	...
	void BeginExtrude(Vector3 moveOffset)
	{
		...
		// Arrays
		Vector3[] verts = new Vector3[vertCount + 16];
		Vector3[] norms = new Vector3[vertCount + 16];
		int[] tris = new int[_mesh.triangles.Length + 24]; // 24 new tris (4 faces = 6*4)
		Vector2[] uvs = new Vector2[vertCount + 16];
		Vector2[] tileUvs = new Vector2[vertCount + 16];

		// Add new verts
		_mesh.vertices.CopyTo(verts, 0);
		_currentFace.GetExtrudeVerts(moveOffset).CopyTo(verts, vertCount);

		// Add new norms
		_mesh.normals.CopyTo(norms, 0);
		_currentFace.GetExtrudeNormals().CopyTo(norms, vertCount);
		...
		{
			// Indices in vert array that correspond to corners of this face
			int tl = vertCount + (i * 4);
			int tr = tl + 1;
			int bl = tl + 2;
			int br = tl + 3;
		...
		// Now move current face
		for(int i = 0; i < 4; i++) // for each selected vert - this will move ONLY face, not attached sides (which use duplicate verts)
		{
			int j =_currentFace.indices[i];
			verts[j] = RoundTo(verts[j] + moveOffset, gridError); // round might not be needed?
		}
		...
		int faceIndex = _currentFace.faceNumber;
		_currentFace = null;
		CalculateFaces();
